...
from kitchen.plotter.macros.jux_data_macros import multiple_dataset_dendrogram_plot, multiple_dataset_fa_plot, multiple_dataset_pca_plot, multiple_dataset_svd_plot, raster_plot, single_cell_session_parallel_view_jux, subtree_summary_trial_avg_jux
...
def main():
    ...
    for activity_period_name, activity_period in {"Short": (-0.25, 0.75), "Medium": (-1, 1.5), }.items():
        for preprocessing_method in ("log-scale", "raw", "z-score",):
            for bin_size in (10/1000, 20/1000,):
                dendrogram_kwargs = {
                    "save_name": f"{activity_period_name}_{preprocessing_method}_{bin_size}",
                    "FEATURE_RANGE": activity_period,
                    "PREPROCESSING_METHOD": preprocessing_method,
                    "BINSIZE": bin_size,
                }
                multiple_dataset_svd_plot(
                    all_datasets, prefix_keyword=f"ALL_SVD_{activity_period_name}_{preprocessing_method}",
                    **dendrogram_kwargs,
                )
                multiple_dataset_pca_plot(
                    all_datasets, prefix_keyword=f"ALL_PCA_{activity_period_name}_{preprocessing_method}",
                    **dendrogram_kwargs,
                )
                multiple_dataset_fa_plot(
                    all_datasets, prefix_keyword=f"ALL_FA_{activity_period_name}_{preprocessing_method}",
                    **dendrogram_kwargs,
                )
